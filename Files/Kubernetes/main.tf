...
resource "null_resource" "push_to_acr" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
     az acr login --name ${azurerm_container_registry.example.name} --expose-token
     docker tag your-local-image:latest ${azurerm_container_registry.example.login_server}//javier0001/104app:latest
     docker push ${azurerm_container_registry.example.login_server}/javier0001/104app:latest
   EOT
  }
}
